...
class TestWatch:
    ...
    def test_watch_输出worker行与卡行(self, mock_request, capsys):
        ...
        mock_request.responses["GET /memories?tag=rounds&limit=1000"] = _ROUNDS_EMPTY
    ...
    # ---- TASK-0060：任务卡行附轮次显示 ----
    def test_watch_卡行附轮次数(self, mock_request, capsys):
        """有 ROUNDS 记录的卡标题后显示 [当前轮/配额]；FBK 标注在其后。"""
        import watch
        mock_request.responses["GET /memories?tag=registry&limit=1000"] = {
            "items": [], "total": 0}
        mock_request.responses["GET /memories?tag=taskboard&limit=1000"] = {
            "items": [
                _card("TASK-0060 pending worker-1 | 有轮次的卡\n目标：x"),
                _card("TASK-0061 pending worker-1 | 无轮次的老卡\n目标：y"),
            ], "total": 2}
        rounds = ("ROUNDS TASK-0060 | precheck=1 milestone=1 review=0 total=3\n"
                  "复杂度：medium\n"
                  "配额：precheck=2 milestone=2 total=5")
        mock_request.responses["GET /memories?tag=feedback&limit=1000"] = {
            "items": [], "total": 0}
        mock_request.responses["GET /memories?tag=rounds&limit=1000"] = {
            "items": [{"id": "r1", "content": rounds, "tags": ["rounds"],
                       "updated_at": "2026-08-26T23:00:00"}], "total": 1}
        watch.cmd_watch(interval=5, comm=False, once=True)
        out = capsys.readouterr().out
        assert "TASK-0060 pending worker-1 12:30 有轮次的卡 [3/5]" in out
        assert "TASK-0061 pending worker-1 12:30 无轮次的老卡 [–]" in out

    def test_watch_卡行轮次越界容错(self, mock_request, capsys):
        """total 超过配额上限（越界）也按实际值显示，不崩溃。"""
        import watch
        mock_request.responses["GET /memories?tag=registry&limit=1000"] = {
            "items": [], "total": 0}
        mock_request.responses["GET /memories?tag=taskboard&limit=1000"] = {
            "items": [_card("TASK-0062 pending worker-2 | 越界的卡\n目标：x")],
            "total": 1}
        rounds = ("ROUNDS TASK-0062 | precheck=2 milestone=2 review=1 total=6\n"
                  "复杂度：medium\n"
                  "配额：precheck=2 milestone=2 total=5")
        mock_request.responses["GET /memories?tag=feedback&limit=1000"] = {
            "items": [], "total": 0}
        ...
        watch.cmd_watch(interval=5, comm=False, once=True)
        out = capsys.readouterr().out
        assert "TASK-0062 pending worker-2 12:30 越界的卡 [6/5]" in out

    def test_watch_卡行FBK与轮次共存(self, mock_request, capsys):
        """rounds 与 open FBK 同时存在：先 [当前轮/配额] 后 [FBK:N]。"""
        import watch
        mock_request.responses["GET /memories?tag=registry&limit=1000"] = {
            "items": [], "total": 0}
        mock_request.responses["GET /memories?tag=taskboard&limit=1000"] = {
            "items": [_card("TASK-0026 pending worker-2 | 双标注的卡\n目标：x")],
            "total": 1}
        rounds = ("ROUNDS TASK-0026 | precheck=1 milestone=1 review=0 total=3\n"
                  "复杂度：medium\n"
                  "配额：precheck=2 milestone=2 total=5")
        fbk_open = ("FBK-0001 feedback TASK-0026 | objection precheck\n"
                    "提出者：designer-1\n目标卡：TASK-0026\n"
                    ...
                    "替代方案：补黑盒用例\n结果：open")
        mock_request.responses["GET /memories?tag=feedback&limit=1000"] = {
            "items": [_fbk(fbk_open)], "total": 1}
        mock_request.responses["GET /memories?tag=rounds&limit=1000"] = {
            "items": [{"id": "r1", "content": rounds, "tags": ["rounds"],
                       "updated_at": "2026-08-26T23:00:00"}], "total": 1}
        watch.cmd_watch(interval=5, comm=False, once=True)
        out = capsys.readouterr().out
        assert "TASK-0026 pending worker-2 12:30 双标注的卡 [3/5] [FBK:1]" in out
