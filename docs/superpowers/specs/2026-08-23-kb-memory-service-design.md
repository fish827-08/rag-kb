...
```
query ──┬─ 向量路：embed → Chroma top-N（N = max(3 × top_k, 30)）
        └─ 关键词路：jieba 分词 → BM25 top-N
                     ↓
       归一化加权融合：score(d) = Σ_w norm_i(d) / 生效路数
...
- Chroma 余弦相似度、BM25 分数值域不同，融合前**每路独立 min-max 归一化**到 [0,1]，
  再按记录实际出现的路数均权平均。**修订说明（2026-09-01）**：原实现用 RRF 排名融合
  `Σ 1/(60+rank)`，将区分度压缩到 ~0.001 噪声级——「某记录双路均第 1」与
  「无关记录双路第 15」融合分几乎重合，真实相关记录（尤其含外语/专有名词，
  如英文歌名、项目专有名词）在 `top_k` 截断时被无关记录挤出，中文 query 查不到
  英文内容。故改为归一化评分加权融合，保留每路真实分数信息；候选窗口由
  `3×top_k` 放宽到 `max(3×top_k, 30)`，缓解小 top_k 下的早截断。
  **N33（2026-09-01）补充**：分母为"该记录出现的路数"（生效路数均权）——跨语言
  （纯英文/日文）记忆在 BM25 路必然缺席（jieba 中文分词无词面重叠），若 ÷ 总路数
  会把向量分砍半再次挤出 top_k；单路出现得原分，双路同现仍均权，同语言零变化。
