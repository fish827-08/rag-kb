...
- ~~不改 RRF 融合公式本身（K=60 维持），只扩路数~~ → **2026-09-01 修订：融合改为归一化评分加权**（下文 §3.3；原因：RRF 排名融合区分度过低，含外语/专有名词记录在 top\_k 截断时被无关记录挤出）
...
```
query ──┬─ 稠密路：embed → Chroma top-N（N = max(3 × top_k, 30)）
        ├─ 关键词路：jieba → BM25 top-N
        └─ 稀疏路（KB_SPARSE_ENABLED）：sparse encode → SparseIndex top-N
                     ↓
       归一化加权融合（2 或 3 路）：score(d) = Σ norm_i(d) / 生效路数
       （每路独立 min-max 归一化到 [0,1]，按记录实际出现的路数均权，保留真实分数区分度）
                     ↓
...
- `rerank` 返回按新分数降序的前 top\_k，每条带 `rerank_score` 字段
...
**降级链**：SparseEmbedder 初始化失败（非 BGE-M3 / 文件缺失 / 加载异常）→ service 记 WARNING，稀疏路自动关闭，检索退回双路（行为等同 sparse\_enabled=false），服务正常启动。

### 3.3 多路融合（retriever.py 改造）

> **2026-09-01 修订（融合算法变更）**：原版本约定「不改 RRF 融合公式（K=60 维持）」。
> 实测发现 RRF 排名融合把区分度压缩到噪声级（双路第 1 的记录融合分与双路第 15 的
> 无关记录仅差 \~0.0006），含外语/专有名词的记录（如英文歌名 Poker Face、项目英文
> 专有名词）在 `top_k` 截断时被无关记录挤出，中文 query 检索不到。修订为归一化评分
> 加权融合，保留每路真实分数区分度。`rrf_fuse` 函数保留（兼容旧测试），hybrid 默认
> 路径改走 `weighted_fuse`。
>
> **2026-09-01 二次修订（N33，生效路数均权）**：首版加权融合按 n 路均权（缺席路计 0 分
> 后 ÷ 总路数），跨语言/异形词记录（纯英文、日文记忆）在 BM25 路必然缺席（jieba 中文分词
> 与异语言内容无词面重叠），向量分被系统性砍半，再次在 `top_k` 截断中排后被挤出。实测
> （BGE-M3）：中文 query「混合检索是如何融合向量和关键词的」向量路对纯英文记忆排名第 1，
> 融合后却被 ÷2 稀释到第 4。故分母改为**该记录实际出现的路数**（生效路数均权）：
> 单路出现得该路归一化原分，双/三路同现仍为均权（同语言场景零行为变化）。

- `weighted_fuse(*ranked_lists, top_k)`（新）：可变参数（2 或 3 路，路数由开关决定）

  - 每路独立 **min-max 归一化**到 \[0,1]（该路 max==min 时：若非零则记为 1，全零记为 0）

  - 融合分 = 出现路数的归一化分**按生效路数均权**（score = Σ norm\_i / 出现路数，N33；无新增配置项）

  - 按融合分降序取 top\_k；记录缺失某路（跨语言场景的 BM25 路）不影响分母，不因缺席受罚

- 融合候选 `candidate = max(3 * top_k, 30)`；稀疏路开启时三路各取 candidate 条融合

- 治理重排（衰减/新鲜度）应用点不变，仍在融合之后、rerank 之前（治理是软信号，rerank 是精排，精排最后）

- **rerank 挂接点**：治理重排后取前 `rerank_top_n`（默认 20）条送 Reranker，输出截断 top\_k；关闭时直接截断（现行为）

### 3.4 N+1 修复与索引持久化（storage.py / bm25.py / retriever.py）

**get\_many（storage.py）**：
...
- `load_corpus(path, valid_ids) -> bool`：文件存在且 id 集合 ⊆ valid\_ids（与 Chroma 一致）→ 加载 tokens 重建 BM25Okapi，返回 True；否则 False（调用方走全量分词重建）
...
**真机基线（2026-08-28 实测，BGE-M3 CPU，n=50）**：

| 组合        | Recall\@1 | Recall\@5 | MRR       | 平均延迟      |
| --------- | --------- | --------- | --------- | --------- |
| 双路（默认基线）  | 0.780     | 0.940     | 0.855     | 131.7 ms  |
| +稀疏三路     | 0.720     | **0.980** | 0.837     | 200.7 ms  |
| +rerank   | **0.880** | **1.000** | **0.935** | 1581.8 ms |
| 三路+rerank | **0.880** | 0.980     | 0.930     | 1618.6 ms |

结论：rerank 收益最大（R\@1 +0.10、semantic 难度 0.588→0.882、R\@5 满分全命中）；稀疏路改善尾召回（R\@5 +0.04）但稀释头排名（R\@1 -0.06），与 rerank 叠加无额外收益。分难度：distractor 全组合满分（RRF 融合对平行干扰结构天然鲁棒）；semantic 是 rerank 的主要受益项。默认关的策略下，推荐按需开 `KB_RERANK_ENABLED=true`（GPU 下 rerank 延迟可从 CPU \~1.5s 降一个量级）。
...
| BGE-M3 稀疏头     | \~0（共享编码器）        | sparse\_linear 1024×1，可忽略                                        |
...
| **N27** | get\_many 批量 + BM25 语料持久化 + retriever N+1 消除 + 回归                                | 标准 |
...
| 风险                                                | 规避                                                                                         |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| reranker 首次下载 2.3GB（国内网络慢）                        | HF\_ENDPOINT 镜像 + `HF_HUB_DISABLE_XET=1`（hf-mirror 的 Xet CAS 401 实测坑）；断网时 rerank 关闭不影响核心功能 |
...
| 评测数据集泄露进生产库                                       | 评测独立 KB\_DATA\_DIR（env 隔离），进程退出数据留存本地（不删，便于复跑）                                             |
| rerank 拖慢交互                                       | 默认关 + top\_n=20 上限；延迟实测不达标（>500ms 总预算）在收口报告标注建议                                            |
